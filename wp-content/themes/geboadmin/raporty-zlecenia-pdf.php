...
$tabela .= '<tr>
<th>L.p.</th><th>Numer</th><th>Nadawca / Odbiorca</th><th>Płatnik</th><th>Pracownik</th><th>Zysk</th>
</tr>';
$lp = 0;
foreach($orders as $order) {
    ...
    $tabela .= '<tr><td>'.++$lp.'</td><td>'.$order->number.'</td><td>'.$order->nadawca.' / '.$order->odbiorca.'</td><td>'.$order->platnik.'</td><td>'.$users[0]->display_name.'</td><td>';
    ...
    $tabela .= ' PLN</td></tr><tr><td colspan="2"</td><td colspan="4" style="text-align:right;background-color:#eee;">';
...
<head>
    ...
    <link rel="stylesheet" href="<?php echo get_template_directory_uri(); ?>/css/print.css" id="link_theme"/>
    <title></title>
</head>
<body>
<style>
...
<table>
    <tr>
        <td colspan="2">Od: <?php echo $_POST['date_start']; ?></td>
        <td colspan="3">Do: <?php echo $_POST['date_end']; ?></td>
    </tr>
    <tr>
        <th></th>
        <th>Lotniczy</th>
    ...
    <tr>
        <td>Liczba zleceń</td>
        <td><?php echo $orders_l_count; ?></td>
        <td><?php echo $orders_m_count; ?></td>
        <td><?php echo $orders_d_count; ?></td>
