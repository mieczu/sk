	...
	<table style="width: 740px; margin: 0 auto; min-height: 100%;">
		...
		<tr>
			<td style="padding: 15px 0 25px; font-size: 14px; border-top: 1px solid #000000" class="b c" colspan="2">
				<?php echo( $results->lang == 1 ? 'Invoice No' : 'Faktura VAT Nr' ); ?> <?php echo $results->numer; ?>
				<br/>
		...
		<tr>
			<td rowspan="3">
				...
				<span class="b"><?php echo $results_client[0]->name; ?></span><br/>
				<span><?php echo $results_client[0]->address; ?></span><br/>
				<span><?php echo $results_client[0]->post_code . ' ' . $results_client[0]->city; ?></span><br/>
				<span><?php echo( $results->lang == 1 ? $results_country[0]->name_en : $results_country[0]->name ); ?></span><br/>
				<span><b><?php echo( $results->lang == 1 ? 'VAT No: ' : 'NIP: ' ); ?></b><?php echo $results_client[0]->kraj . $results_client[0]->nip; ?></span><br/>
			</td>
			<td>
        <span
		...
		<tr>
			<td>
				<span
					class="bl b"><?php echo( $results->lang == 1 ? 'Date of Service:' : 'Data wykonania usługi:' ); ?></span>
		...
		<tr>
			<td>
				...
				<span
					class="bl"><?php echo $results_order[0]->number . inicialy( $results_user[0]->display_name ); ?></span>
			</td>
		</tr>
		<tr>
			<td>
			...
			<td colspan="2">
				<table class="c" cellspacing="0" style="width:100%;">
					...
					foreach ( $results_sub as $sub ) {
						?>
						<tr>
							<td><?php echo ++ $count; ?></td><!-- LP-->
							<td class="l"><?php echo $sub->name; ?></td><!-- Tytułem-->
							<td><?php echo $sub->quantity; ?></td><!-- Ilość-->
							<td>szt.</td><!--  -->
							...
							if ( $results->currency == $sub->currency ) {
								...
								?>
								<td><?php echo number_format( ( $sub->value / 10000 ) * $sub->quantity, 2, ',', ' ' ); ?><?php echo $sub->currency; ?></td><!-- Wartość bez podatku-->
								<td><?php echo( ! is_numeric( $sub->vat ) ? $np = 'NP' : $sub->vat / 10000 ); ?></td><!-- Stawka VAT %-->
								<td>
									<?php
								...
								////////////////////////////////////////////////////////////////
								$vat[ $sub->vat ]['total'] += round( ( $sub->value / 10000 ) * $sub->quantity * $exchange, 2 );
								$currency = $results->currency;

								?>
								<td><?php echo number_format( ( $sub->value / 10000 ) * $sub->quantity * $exchange, 2, ',', ' ' ); ?><?php echo $results->currency; ?></td><!-- Wartość bez podatku-->
								<td><?php echo( ! is_numeric( $sub->vat ) ? $np = 'NP' : ( $sub->vat / 10000 ) ); ?></td><!-- Stawka VAT-->
								<td><?php echo number_format( ( $sub->vat / 1000000 ) * ( $sub->value / 10000 ) * $sub->quantity * $exchange * $exchange_vat, 2, ',', ' ' ) . ' ' . $results->vat_currency; ?></td><!-- Wartość VAT-->
								<td>
									<?php
									if ( $results->currency == $results->vat_currency ) {
										//                                echo number_format(((($sub->vat / 1000000) * ($sub->value / 10000) * $sub->quantity) + (($sub->value / 10000) * $sub->quantity)) * $exchange, 2, ',', ' ').' '.$results->currency;
										echo number_format( round( ( $sub->vat / 1000000 ) * ( $sub->value / 10000 ) * $sub->quantity * $exchange * $exchange_vat, 2 ) + round( ( $sub->value / 10000 ) * $sub->quantity * $exchange, 2 ), 2, ',', ' ' ) . ' ' . $results->currency;
										//
										//                                echo round(($sub->value / 10000) * $sub->quantity * $exchange, 2);
										//                                echo '+';
										//                                echo round(($sub->vat / 1000000) * ($sub->value / 10000) * $sub->quantity * $exchange * $exchange_vat,2);
										//                                echo '=';
										//                                echo round(($sub->vat / 1000000) * ($sub->value / 10000) * $sub->quantity * $exchange * $exchange_vat,2)+round(($sub->value / 10000) * $sub->quantity * $exchange, 2);
					...
					<tr>
						...
						<td colspan="4">
							<?php
							if ( ( $results_order[0]->typ == 'import' ) && ( $results_order[0]->transport != 'Drogowy' ) ) {
								?>
								uwagi: * art.83 ust 1 pkt 23 ustawy o VAT
								<?php
							}
							if ( ( $results_order[0]->typ == 'export' ) && ( $results_order[0]->transport != 'Drogowy' ) ) {
								?>
								uwagi: * art.83 ust 1 pkt 21b ustawy o VAT
							<?php } ?>
						</td>
					</tr>
					</tbody>
			...
			<td style="page-break-inside: avoid;" colspan="2">
				<table>
					<tr>
						...
						<td class="l b"><?php echo number_format( $total, 2, ',', ' ' ) . ' ' . $currency; ?></td>
					</tr>
					<tr>
						<td class="r"><?php echo( $results->lang == 1 ? 'Say' : 'Do zapłaty' ); ?>:</td>
					...
					<tr>
						<td class="r"><?php echo( $results->lang == 1 ? 'Sender/Receiver' : 'Nadawca/Odbiorca' ); ?>:
						</td>
		...
		<tfoot>
		<tr>
			<td class="c bt f8 print" colspan="2">Działamy w oparciu o Ogólne Polskie Warunki Spedycyjne dostępne na
				stronie
				www.pisil.pl
			</td>
		</tr>
