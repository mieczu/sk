...
if(is_numeric($_REQUEST['id_invoice'])) {
    $results_sub = $wpdb->get_results('SELECT * FROM wp_czm_invoices_items_to_invoices WHERE id_invoice='.$_REQUEST['id_invoice'], OBJECT);
}elseif(is_numeric($_REQUEST['id'])){
    $results_sub = $wpdb->get_results('SELECT * FROM wp_czm_korekty_items_to_invoices WHERE id_invoice='.$_REQUEST['id'], OBJECT);
}
